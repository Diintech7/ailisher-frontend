...
export default function Configuration() {
  ...
  const fetchConfig = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`http://localhost:4000/api/config/clients/${clientId}`, {
      ...
      await Promise.all(
        configs.map(async (cfg) => {
          if (cfg && cfg.sourcetype) {
            try {
              const res = await fetch(`http://localhost:4000/api/config/clients/${clientId}/config/${cfg.sourcetype}/expired-models`, {
                headers: getAuthHeaders()
              });
  ...
  const handleAdd = async (sourcetype, model) => {
    try {
      setLoading(true);
      await fetch(`http://localhost:4000/api/config/clients/${clientId}/model`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const handleUpdate = async (model) => {
    try {
      setLoading(true);
      await fetch(`http://localhost:4000/api/config/clients/${clientId}/model/${editModel.sourcetype}/${editModel.key}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
  ...
  const handleDelete = async (model, sourcetype) => {
    ...
    try {
      setLoading(true);
      await fetch(`http://localhost:4000/api/config/clients/${clientId}/model/${sourcetype}/${model.key}`, {
        method: 'DELETE',
        headers: getAuthHeaders()
      });
      await fetchConfig();
