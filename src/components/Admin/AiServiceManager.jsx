...
const AiServiceManager = () => {
  ...
  const fetchServices = async () => {
    try {
      setLoading(true);
      const response = await fetch((process.env.REACT_APP_API_URL || 'http://localhost:4000') + '/api/ai');
  ...
  const fetchApiKey = async (serviceName) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/ai/${serviceName}/apikey`);
      const data = await response.json();
  ...
  const handleUpdateTaskPreferences = async (serviceName, taskPreferences) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/ai/${serviceName}/preferences`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleUpdateServiceConfig = async (serviceName, serviceConfig) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/ai/${serviceName}/config`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleToggleService = async (serviceName) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/ai/${serviceName}/toggle`, {
        method: 'PUT',
      });
  ...
  const handleDeleteService = async (serviceName) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/ai/${serviceName}`, {
        method: 'DELETE',
      });
