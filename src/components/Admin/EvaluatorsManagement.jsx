...
const EvaluatorsManagement = () => {
  ...
  const handleDelete = async (evaluatorId) => {
    if (window.confirm('Are you sure you want to delete this evaluator?')) {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/evaluators/${evaluatorId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/evaluators/${selectedEvaluator._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleApproveSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/clients/CLI677117YN7N/mobile/evaluations/addexistinguserasevaluator', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleVerify = async (evaluatorId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/evaluators/${evaluatorId}/verify`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleToggleStatus = async (evaluatorId, currentStatus) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/evaluators/${evaluatorId}/togglestatus`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
