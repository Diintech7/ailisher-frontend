...
const AISWBPrintModal = ({ isOpen, onClose, topicId }) => {
  ...
  const fetchSetsForPrint = async () => {
    ...
    try {
      ...
      console.log('📡 API URL:', `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets`);

      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets`, {
        headers: {
  ...
  const generateQRCodeForQuestion = async (questionId) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/qr/questions/${questionId}/qrcode?${queryParams}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
