...
const AssetView = () => {
  ...
  useEffect(() => {
    const fetchAssets = async () => {
      try {
        ...
        const videosEndpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/video-assets/${itemType}/${itemId}/videos?isWorkbook=${isWorkbook}`
        const pyqsEndpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/pyq-assets/${itemType}/${itemId}/pyqs?isWorkbook=${isWorkbook}`
        const subjectiveEndpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/subjective-assets/${itemType}/${itemId}/question-sets?isWorkbook=${isWorkbook}`
        const objectiveEndpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/objective-assets/${itemType}/${itemId}/question-sets?isWorkbook=${isWorkbook}`

        // Determine endpoint based on item type for summaries
        let baseEndpoint = ""
        if (itemType === "book") {
          baseEndpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${workbookId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}`
        } else if (itemType === "chapter") {
          baseEndpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${workbookId}/chapters/${chapterId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}/chapters/${chapterId}`
        } else if (itemType === "topic") {
          baseEndpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${workbookId}/chapters/${chapterId}/topics/${topicId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}/chapters/${chapterId}/topics/${topicId}`
        } else if (itemType === "subtopic") {
          baseEndpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${workbookId}/chapters/${chapterId}/topics/${topicId}/subtopics/${subtopicId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}/chapters/${chapterId}/topics/${topicId}/subtopics/${subtopicId}`
        }
  ...
  useEffect(() => {
    const fetchItemDetails = async () => {
      ...
      try {
        ...
        if (itemType === "book") {
          endpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${workbookId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}`
        } else if (itemType === "chapter") {
          endpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${workbookId}/chapters/${chapterId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}/chapters/${chapterId}`
        } else if (itemType === "topic") {
          endpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${workbookId}/chapters/${chapterId}/topics/${topicId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}/chapters/${chapterId}/topics/${topicId}`
        } else if (itemType === "subtopic") {
          endpoint = isWorkbook
            ? `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${workbookId}/chapters/${chapterId}/topics/${topicId}/subtopics/${subtopicId}`
            : `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/assets/${bookId}/chapters/${chapterId}/topics/${topicId}/subtopics/${subtopicId}`
        }
  ...
  const handleDeleteQuestion = async () => {
    ...
    try {
      const token = Cookies.get("usertoken")
      const endpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/subjective-assets/questions/${deletingQuestion._id}`
  ...
  const handleDeleteObjectiveQuestion = async () => {
    ...
    try {
      const token = Cookies.get("usertoken")
      const endpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/objective-assets/questions/${deletingObjectiveQuestion._id}`
  ...
  const handleDeleteVideo = async (videoId) => {
    try {
      const token = Cookies.get("usertoken")
      const endpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/video-assets/videos/${videoId}`
  ...
  const handleDeletePYQ = async (pyqId) => {
    try {
      const token = Cookies.get("usertoken")
      const endpoint = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/pyq-assets/pyqs/${pyqId}`
