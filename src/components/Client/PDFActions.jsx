...
const PDFActions = ({ item, onEmbeddingComplete }) => {
  ...
  const checkEmbeddingStatus = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/pdf-processing/check-embeddings/${item._id}`, {
        headers: {
  ...
  const loadChatHistory = async () => {
    try {
      const token = Cookies.get("usertoken")
      const response = await fetch(`http://localhost:4000/api/pdf-processing/chat-history/${item._id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleCreateEmbeddings = async () => {
    ...
    try {
      const token = Cookies.get("usertoken")
      const response = await fetch(`http://localhost:4000/api/pdf-processing/create-embeddings/${item._id}`, {
        method: "POST",
        headers: {
  ...
  const handleSendQuestion = async () => {
    ...
    try {
      const token = Cookies.get("usertoken")
      const response = await fetch(`http://localhost:4000/api/pdf-processing/chat/${item._id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
