...
const PDFActions = ({ item, onEmbeddingComplete }) => {
  ...
  const checkEmbeddingStatus = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/pdf-processing/check-embeddings/${item._id}`, {
        headers: {
  ...
  const loadChatHistory = async () => {
    try {
      const token = Cookies.get("usertoken")
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/pdf-processing/chat-history/${item._id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleCreateEmbeddings = async () => {
    ...
    try {
      const token = Cookies.get("usertoken")
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/pdf-processing/create-embeddings/${item._id}`, {
        method: "POST",
        headers: {
  ...
  const handleSendQuestion = async () => {
    ...
    try {
      const token = Cookies.get("usertoken")
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/pdf-processing/chat/${item._id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        <div className="flex-1 overflow-y-auto mb-4 border border-gray-200 rounded-lg p-4 bg-gray-50">
          {chatMessages.map((msg, index) => (
            <div key={index} className={`mb-4 ${msg.type === "user" ? "text-right" : "text-left"}`}>
              <div
                className={`inline-block max-w-3xl p-3 rounded-lg ${msg.type === "user"
    ...
    <>
      <div className="flex gap-2 mt-2">
        ...
        <button
          ...
          className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors ${embeddingStatus === "success"
