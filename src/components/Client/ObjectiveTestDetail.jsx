            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  {Object.values(questionsByLevel).reduce((t, q) => t + q.length, 0)} questions • Objective Type
                </p>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-lg font-semibold text-gray-900">{Object.values(questionsByLevel).reduce((t, q) => t + q.length, 0)}</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3">
            ...
            {getLevelQuestions(activeLevel).length === 0 ? (
              <div className="text-center py-12 flex flex-col items-center justify-center">
                ...
                <p className="text-gray-600 mb-6">
                  There are no questions for the {activeLevel === "L1" ? "Beginner" : activeLevel === "L2" ? "Intermediate" : "Advanced"} level yet.
                </p>
              </div>
            ) : (
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2">
                            <button
                              ...
                              onClick={() => {
                                ...
                                setFormData({
                                  ...
                                  solution: question.solution || { type: "text", text: "", video: { url: "", title: "", description: "", duration: 0 }, image: { url: "", caption: "" } },
                                });
                                setShowQuestionModal(true);
                              }}
                            >
                            ...
                            <button
                              ...
                              onClick={() => handleDeleteObjectiveQuestion(question._id || question.id)}
                            >
                              <Trash2 size={18} />
                            </button>
                          </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {question.options.map((option, optIndex) => (
                                ...
                                >
                                  <div className="flex items-center space-x-3">
                                    <span className={`text-sm font-bold px-2 py-1 rounded-full ${optIndex === question.correctAnswer ? "bg-green-600 text-white" : "bg-gray-100 text-gray-500"}`}>
                                      {String.fromCharCode(65 + optIndex)}
                                    </span>
                                    <span className={`font-medium ${optIndex === question.correctAnswer ? "text-green-900" : "text-gray-900"}`}>{option}</span>
                                    {optIndex === question.correctAnswer && <CheckCircle size={20} className="text-green-600 ml-auto" />}
                                  </div>
                                </div>
                              ))}
                            </div>
      ...
      {showQuestionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">{editingQuestion ? "Edit Objective Question" : "Add Objective Question"}</h3>
              <button
            ...
            <ObjectiveQuestionForm
              ...
              index={0}
              canRemove={false}
              onQuestionChange={(_, updated) => setFormData(prev => ({ ...prev, ...updated }))}
              fixedDifficulty={formData.difficulty}
            />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm text-gray-700 mb-1">Estimated Time (mins)</label>
                <input type="number" min={0} value={formData.estimatedTime}
                       onChange={e => setFormData(prev => ({ ...prev, estimatedTime: parseInt(e.target.value) || 0 }))}
                       className="w-full px-3 py-2 border border-gray-300 rounded-md" />
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Positive Marks</label>
                <input type="number" min={0} step="0.1" value={formData.positiveMarks}
                       onChange={e => setFormData(prev => ({ ...prev, positiveMarks: Number(e.target.value) }))}
                       className="w-full px-3 py-2 border border-gray-300 rounded-md" />
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Negative Marks</label>
                <input type="number" min={0} step="0.1" value={formData.negativeMarks}
            ...
            <div className="mt-4">
              <label className="block text-sm text-gray-700 mb-1">Solution Type</label>
              <select
                value={formData.solution?.type || "text"}
                onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, type: e.target.value } }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
              >
              ...
              {formData.solution?.type === "text" && (
                <div className="mt-2">
                  <label className="block text-sm text-gray-700 mb-1">Solution Text</label>
                  <textarea
                    rows={3}
                    value={formData.solution?.text || ""}
                    onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, text: e.target.value } }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md"
                  />
                </div>
              )}
                ...
                <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, video: { ...prev.solution.video, url: e.target.value } } }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, video: { ...prev.solution.video, title: e.target.value } } }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    />
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, video: { ...prev.solution.video, description: e.target.value } } }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, video: { ...prev.solution.video, duration: parseInt(e.target.value) || 0 } } }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    />
                  </div>
                </div>
                ...
                <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, image: { ...prev.solution.image, url: e.target.value } } }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData(prev => ({ ...prev, solution: { ...prev.solution, image: { ...prev.solution.image, caption: e.target.value } } }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md"
                    />
                  </div>
                </div>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                ...
                onClick={() => (editingQuestion ? handleUpdateObjectiveQuestion() : handleAddObjectiveQuestion())}
              >
