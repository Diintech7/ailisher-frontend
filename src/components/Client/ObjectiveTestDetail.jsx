...
export default function ObjectiveTestDetail() {
  ...
  const openScoreboard = async () => {
    try {
      ...
      setScoreboard({ summary: summary || null, rows: Array.isArray(data) ? data : [] });
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex flex-wrap gap-8 mt-5">
              <div className="inline-flex rounded-md border border-gray-300 overflow-hidden">
                <button
                  ...
                  className={`px-4 py-2 text-sm ${activeView === 'questions' ? 'bg-gray-800 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
                >
                  Questions
                </button>
                <button
                  onClick={async () => { if (activeView !== 'scoreboard') { setActiveView('scoreboard'); await openScoreboard(); } }}
                  className={`px-4 py-2 text-sm flex items-center ${activeView === 'scoreboard' ? 'bg-gray-800 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
                >
      ...
      {activeView === 'scoreboard' && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
            <div className="p-6">
              <h3 className="text-xl font-semibold mb-4">First Attempt Scoreboard</h3>
              {scoreboardLoading ? (
                  ...
                  {scoreboard.summary && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      <div className="p-4 bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-xl border shadow-sm">
                        <div className="text-xs text-yellow-700">Top Score</div>
                        <div className="text-2xl font-semibold text-yellow-900">{scoreboard.summary.topScore}</div>
                      </div>
                      <div className="p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border shadow-sm">
                        ...
                        <div className="text-2xl font-semibold text-blue-900">{scoreboard.summary.averageScore}</div>
                      </div>
                      <div className="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-xl border shadow-sm">
                        <div className="text-xs text-green-700">Total Users Appeared</div>
                        <div className="text-2xl font-semibold text-green-900">{scoreboard.summary.totalUsersAppeared}</div>
                      </div>
                    </div>
                  )}
                    ...
                    <table className="min-w-full border rounded-lg overflow-hidden">
                      <thead className="bg-gray-100">
                        <tr>
                          ...
                          <th className="text-left px-3 py-2 border">Marks</th>
                          <th className="text-left px-3 py-2 border">Completion Time</th>
                          <th className="text-left px-3 py-2 border">Submitted At</th>
                        </tr>
                      </thead>
                      <tbody>
                        {scoreboard.rows.length === 0 ? (
                          <tr>
                            <td colSpan={5} className="text-center py-6">No data</td>
                          </tr>
                        ) : (
                          scoreboard.rows.map((row) => (
                            <tr key={`${row.userId}-${row.rank}`} className="odd:bg-white even:bg-gray-50">
                              <td className="px-3 py-2 border font-semibold">{row.rank}</td>
                              <td className="px-3 py-2 border">{row.user?.name || row.user?.email || row.user?.mobile || String(row.userId).slice(-6)}</td>
                              <td className="px-3 py-2 border">{row.totalMarksEarned}</td>
                              <td className="px-3 py-2 border">{row.completionTime || '-'}</td>
                              <td className="px-3 py-2 border">{row.submittedAt ? new Date(row.submittedAt).toLocaleString() : '-'}</td>
                            </tr>
                          ))
                        )}
                      </tbody>
