    ...
    if (isOpen && topicId) {
      ...
      const fetchSubjective = async () => {
        try {
          ...
          if (!token) return;
          const res = await fetch(`https://aipbbackend-yxnh.onrender.com/api/aiswb/topic/${topicId}/sets`, {
          ...
          if (data.success) {
            ...
            const setsWithQuestions = await Promise.all((data.sets || []).map(async set => {
              ...
              const questions = await Promise.all(questionIds.map(async (qid) => {
                try {
                  const qres = await fetch(`https://aipbbackend-yxnh.onrender.com/api/aiswb/questions/${qid}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                  });
