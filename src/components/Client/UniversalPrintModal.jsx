    ...
    if (isOpen && topicId) {
      ...
      const fetchSubjective = async () => {
        try {
          ...
          if (!token) return;
          const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets`, {
          ...
          if (data.success) {
            ...
            const setsWithQuestions = await Promise.all((data.sets || []).map(async set => {
              ...
              const questions = await Promise.all(questionIds.map(async (qid) => {
                try {
                  const qres = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/questions/${qid}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                  });
