...
const BulkUploadAISWBModal = ({ isOpen, onClose, topicId, existingSets = [], onUploadComplete }) => {
  ...
  const createSet = async (setName) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets`,
      {
        method: 'POST',
        headers: {
  ...
  const createQuestion = async (setId, questionObj) => {
    const token = Cookies.get('usertoken');
    const response = await fetch(
      `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets/${setId}/questions`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
