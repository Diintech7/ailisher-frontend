...
const DataStore = ({ type }) => {
  ...
  const getApiEndpoint = () => {
    if (type === "book" && bookId) {
      return `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/book/${bookId}`
    } else if (type === "chapter" && bookId && chapterId) {
      return `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/chapter/${chapterId}`
    } else if (type === "topic" && bookId && chapterId && topicId) {
      return `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/topic/${topicId}`
    } else if (type === "subtopic" && bookId && chapterId && topicId && subtopicId) {
      return `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/subtopic/${subtopicId}`
    }
    return null
  }
  ...
  const updateEmbeddingStatusInDB = async (itemId, isEmbedded, embeddingCount = 0) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/update-embedding-status/${itemId}`, {
        method: "PATCH",
        headers: {
  ...
  const refreshS3Urls = async (items) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/refresh-s3-urls`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const checkEmbeddingStatusForItems = async (pdfItems) => {
    ...
    const statusPromises = pdfItems.map(async (item) => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/enhanced-pdf-embedding/check-embeddings/${item._id}`, {
          headers: {
        ...
        try {
          const healthResponse = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/enhanced-pdf-chat/chat-health/${item._id}`, {
            headers: {
              ...(token && { Authorization: `Bearer ${token}` }),
            },
          })
  ...
  const refreshEmbeddingStatus = async (itemId) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/enhanced-pdf-embedding/check-embeddings/${itemId}`, {
        headers: {
          ...(token && { Authorization: `Bearer ${token}` }),
        },
      })
      ...
      let healthData = { success: false, status: { chatAvailable: false } }
      try {
        const healthResponse = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/enhanced-pdf-chat/chat-health/${itemId}`, {
          headers: {
            ...(token && { Authorization: `Bearer ${token}` }),
          },
        })
  ...
  const fetchAIGuidelines = async () => {
    ...
    try {
      const token = Cookies.get('usertoken')
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiguidelines/${bookId}`, {
    ...
    try {
      ...
      if (uploadType === "file") {
        const uploadPromises = selectedFiles.map(async (file) => {
          // Get S3 upload URL
          const uploadUrlResponse = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/upload-s3`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
  ...
  const handleCreateEmbeddings = async (itemId, forceReEmbed = false) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/enhanced-pdf-embedding/create-embeddings/${itemId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token && { Authorization: `Bearer ${token}` }),
  ...
  const handleSendClick = async () => {
    ...
    try {
      ...
      if (currentChatItem._id === "knowledge-base") {
        response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/enhanced-pdf-chat/chat-book-knowledge-base/${bookId}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...(token && { Authorization: `Bearer ${token}` }),
        ...
        response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/enhanced-pdf-chat/chat/${currentChatItem._id}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...(token && { Authorization: `Bearer ${token}` }),
