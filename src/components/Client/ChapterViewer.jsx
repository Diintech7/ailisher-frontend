...
const ChapterViewer = () => {
  ...
  useEffect(() => {
    const fetchChapterData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/qrcode/book-data/${bookId}/chapters/${chapterId}`);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 mb-8">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-3/4 lg:w-4/5">
            <div className="mb-2">
              <span className="text-sm text-gray-500">From "{chapterData.book?.title || 'Unknown Book'}"</span>
            </div>
      ...
      <div>
        ...
        {!chapterData.datastoreItems || chapterData.datastoreItems.length === 0 ? (
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
            <p className="text-gray-600">No datastore items available for this chapter</p>
          </div>
        ) : (
