...
const BookItem = ({ book, onClick, onToggleHighlight, onToggleTrending, currentUser, onEdit, categoryMappings }) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://aipbbackend.onrender.com/api/books/${book._id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (coverImage) {
        try {
          // Get presigned URL
          const uploadUrlResponse = await fetch('https://aipbbackend.onrender.com/api/books/cover-upload-url', {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${token}`,
...
const AIBooks = () => {
  ...
  const fetchBooks = async () => {
    ...
    try {
      ...
      const booksResponse = await fetch(`https://aipbbackend.onrender.com/api/books?${queryParams}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      if (booksData.success) {
        ...
        if (booksData.categoryOrders) {
          ...
          booksData.books.forEach(book => {
            if (!orders[book.mainCategory] || book.categoryOrder > orders[book.mainCategory]) {
  ...
  const groupedBooks = books.reduce((acc, book) => {
    ...
    const subCategory = book.subCategory === 'Other' && book.customSubCategory 
      ? book.customSubCategory 
      : book.subCategory;
  ...
  const handleCategoryOrderUpdate = async (mainCategory, order) => {
    try {
      ...
      const response = await fetch(`https://aipbbackend.onrender.com/api/books/categories/${mainCategory}/order`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {loading ? (
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          ...
          <h3 className="text-xl font-medium text-gray-800 mb-2">No books yet</h3>
          <p className="text-gray-600 mb-6">Create your first AI book to get started</p>
          <button 
            onClick={handleAddBook}
            ...
            {Object.entries(groupedBooks)
              ...
              <div key={mainCategory} className="space-y-6">
                  ...
                  <div className="flex items-center gap-4">
                    {editingCategory === mainCategory ? (
                      <input
                        ...
                        onKeyPress={(e) => handleOrderKeyPress(e, mainCategory)}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {getBooksToDisplay(mainCategory, subCategories).map((book) => (
                    <BookItem 
                      key={book._id} 
                      book={book} 
