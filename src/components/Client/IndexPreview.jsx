...
const uploadToS3 = async (file, title) => {
  try {
    ...
    const presignRes = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/datastores/upload-s3`, {
      method: 'POST',
      headers: {
...
const IndexPreview = ({
  ...
  const handleSaveToDatastore = async () => {
    ...
    try {
      ...
      // Now save to backend
      const saveResponse = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/books/${bookId}/save-split-pdfs`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
