...
const AddChapterModal = ({ isOpen, onClose, bookId, onAdd }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/books/${bookId}/chapters`, {
        method: 'POST',
        headers: {
...
const EditBookModal = ({ isOpen, onClose, book, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/books/${book._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
...
const BookDetail = () => {
  ...
  const fetchBookDetails = async () => {
    ...
    try {
      ...
      const bookPromise = fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/books/${bookId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      const chaptersPromise = fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/books/${bookId}/chapters`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const confirmDeleteBook = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/books/${bookId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
