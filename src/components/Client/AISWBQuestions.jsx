...
const AISWBQuestions = ({ topicId, selectedSet, onBack }) => {
  ...
  const refreshQuestions = async () => {
    ...
    try {
      ...
      const questionsPromises = questionIds.map(async (questionId) => {
        try {
          const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/questions/${questionId}`, {
            headers: {
  ...
  const handleAddQuestion = async (newQuestion) => {
    try {
      ...
      console.log('API endpoint:', `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets/${selectedSet.id}/questions`);

      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets/${selectedSet.id}/questions`, {
        method: 'POST',
        headers: {
  ...
  const handleEditQuestion = async (editedQuestion) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/questions/${editedQuestion.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleDeleteQuestion = async (questionId) => {
    if (window.confirm('Are you sure you want to delete this question?')) {
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/topic/${topicId}/sets/${selectedSet.id}/questions/${questionId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  const generateFallbackQRCode = (question, format = qrFormat, size = qrSize, includeAns = includeAnswers) => {
    try {
      ...
      if (format === 'text') {
        ...
        qrText = `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/view/questions/${question.id}`;
        qrContent = qrText;
      } else {
        // Default to JSON
  ...
  const fetchQRCode = async (question, format = qrFormat, size = qrSize, includeAns = includeAnswers) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/aiswb/qr/questions/${question.id}/qrcode?${queryParams}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
