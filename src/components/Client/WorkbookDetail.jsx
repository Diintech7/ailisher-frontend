...
const AddChapterModal = ({ isOpen, onClose, workbookId, onAdd }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/workbooks/${workbookId}/chapters`, {
        method: 'POST',
        headers: {
...
const EditWorkbookModal = ({ isOpen, onClose, workbook, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/workbooks/${workbook._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
...
const WorkbookDetail = () => {
  ...
  const fetchWorkbookDetails = async () => {
    ...
    try {
      ...
      const workbookResponse = await fetch(`http://localhost:4000/api/workbooks/${workbookId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      // Fetch chapters
      const chaptersResponse = await fetch(`http://localhost:4000/api/workbooks/${workbookId}/chapters`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
