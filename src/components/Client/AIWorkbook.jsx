...
const WorkbookItem = ({ workbook, onClick, onEdit, onUpdateWorkbook, onToggleEnabled, onDeleted }) => {
  ...
  const handleDeleteWorkbook = async (id) => {
    ...
    const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${id}`, {
      method: 'DELETE',
      headers: {
  ...
  const handleToggleHighlight = async (e) => {
    ...
    try {
      ...
      console.log(options)
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${workbook._id}/highlight`, options);
  ...
  const handleToggleTrending = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${workbook._id}/trending`, options);
      const data = await response.json();
      if (data.success) {
...
const AddWorkbookModal = ({ isOpen, onClose, onAdd, currentUser, categoryMappings, onCategoriesUpdated }) => {
  ...
  const handleCreateSubcategory = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/categories/${currentCat._id}/subcategories`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (coverImage) {
        // Get presigned URL
        const uploadUrlResponse = await fetch((process.env.REACT_APP_API_URL || 'http://localhost:4000') + '/api/workbooks/cover-upload-url', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
      ...
      // Send to backend
      const response = await fetch((process.env.REACT_APP_API_URL || 'http://localhost:4000') + '/api/workbooks', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
...
const EditBookModal = ({ isOpen, onClose, onEdit, book, currentUser, categoryMappings, onCategoriesUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${book._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
...
const AIWorkbook = () => {
  ...
  const handleToggleEnabled = async (workbookId, isEnabled) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/workbooks/${workbookId}`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
