...
const PopularSubmissions = ({ questionId }) => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Filter submissions for this specific question and popularity status
        ...
        console.log('Filtered Popular Submissions:', filteredSubmissions);
  ...
  const handlePopularityUpdate = async (answerId, newStatus, reason) => {
    try {
      const response = await userAnswerService.updatePopularity(answerId, newStatus, reason);
      if (response.success) {
        toast.success('Popularity status updated successfully');
    ...
    <div className="space-y-6">
      ...
      {submissions.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-500">No popular submissions found for this question</p>
        </div>
      ) : (
