...
const RejectedSubmissions = ({ questionId }) => {
  ...
  const handleStatusUpdate = async (answerId, newStatus, reason) => {
    try {
      const response = await userAnswerService.updateMainStatus(answerId, newStatus, reason);
      if (response.success) {
    ...
    <div className="space-y-6">
      ...
      {submissions.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-500">No rejected submissions found for this question</p>
        </div>
      ) : (
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Rejected on {formatDateTime(submission.rejectedAt)}
                </p>
              </div>
                 ...
                 {submission.rejectionReason && (
                  <div className="bg-red-50 p-4 rounded-lg">
                    ...
                    <div className="bg-white p-4 rounded-lg border border-gray-200">
                      <p className="text-sm text-gray-800 whitespace-pre-wrap">{submission.rejectionReason}</p>
                    </div>
                  </div>
                )}
