...
export default function SubjectiveTestDetail() {
  ...
  const fetchTestDetails = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/subjectivetest-questions/${testId}`,
        {
      ...
      const testResponse = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/subjectivetests/${testId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleEditSubjectiveQuestion = async (editedQuestion) => {
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_API_URL || 'http://localhost:4000'}/api/subjectivetest-questions/${questionId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
